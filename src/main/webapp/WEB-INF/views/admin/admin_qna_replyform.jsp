	...
	<div class="container">
		...
		<main class="clearfix mt-1" style="height: 500px">
			...
			<section class="col-10 h-100 bg-white float-left">
				...
				<form id="write_form">
					...
					<table class="table table-boardered">
						...
						<input type="hidden" id="originNo" name="originNo"
							value="${detail_dto.originNo }">
						<input type="hidden" id="groupOrd" name="groupOrd"
							value="${detail_dto.groupOrd }">
						<input type="hidden" id="groupLayer" name="groupLayer"
							value="${detail_dto.groupLayer }">
						<tr>
							<th>문의 유형</th>
							<td><select id="qa_question_category"
								name="qa_question_category">
									<option value="상품"
										<c:if test="${detail_dto.qa_question_category eq '상품'}">selected</c:if>>상품</option>
									<option value="배송"
										<c:if test="${detail_dto.qa_question_category eq '배송'}">selected</c:if>>배송</option>
									<option value="교환/반품"
										<c:if test="${detail_dto.qa_question_category eq '교환/반품'}">selected</c:if>>교환/반품</option>
									<option value="이벤트/쿠폰"
										<c:if test="${detail_dto.qa_question_category eq '이벤트/쿠폰'}">selected</c:if>>이벤트/쿠폰</option>
									<option value="기타"
										<c:if test="${detail_dto.qa_question_category eq '기타'}">selected</c:if>>기타</option>
							</select></td>
						</tr>
						<tr>
							<th>제목</th>
							<td><input type="text" id="qa_question_title"
								name="qa_question_title" maxlength="20" class="form-control"
								value="RE:&nbsp;${detail_dto.qa_question_title}"> <label
								for="qa_question_title" id="qa_question_title_label"
								class="write_label"></label></td>
						</tr>
						<tr>
							<th>문자메세지</th>
							<!--수정하거나 삭제할 예정-->
							<td><input readonly type="text"
								placeholder="${detail_dto.member_id}" cols="100"
								style="width: 200px;"> <input type="checkbox"
								id="sms_yn" value="Y">&nbsp;답변 수신을 문자메세지로 받겠습니다.</td>
						</tr>
						<tr>
							<th>작성자</th>
							<td><input id="member_nick" name="member_nick" type="text"
								readonly="readonly" value="${login_info.member_nick}" cols="100"
						...
						<tr>
							...
							<td><input class="form-check-input" type="checkbox"
								name="open_yn" id="open_yn"
								<c:if test="${detail_dto.open_yn eq 'true'}">checked</c:if>>
								<label class="form-check-label">비밀글 설정</label></td>
						</tr>
						<tr>
							<th>내용</th>
							<td>
								<div>
									<textarea class="form-contol" id="qa_question_content" name="qa_question_content">				
								
								---------- 원본 ----------
								${detail_dto.qa_question_content}
								</textarea>
								
								</div> <script type="text/javascript">
									CKEDITOR.replace("qa_question_content");
								</script> <label for="qa_question_content" id="qa_question_content_label"
				...
				<!-- 수정 완료, 수정 취소 버튼 -->
				<div>
					<button id="write_btn" class="btn btn-info float-right">
						작성 완료</button>
					<a
						href="${pageContext.request.contextPath}/admin/admin_qna_list?qa_question_id=${detail_dto.qa_question_id}">
...
<script type="text/javascript">
	$(document).ready(function() { let open_yn = null;
		...
		if ($.trim($("#qa_question_title").val()) == "") {
			...
			return;
				...
				$("#qa_question_title_label").text("");
					...
					$.post(
							"${pageContext.request.contextPath}/admin/admin_qna_replyInsert",
							{
