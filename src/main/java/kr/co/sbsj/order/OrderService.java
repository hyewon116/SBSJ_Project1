...
public class OrderService {
	...
	public int insert( OrderMainDTO dto, int buyNowYN ) {
		int successCount = 0;
		successCount = dao.insertOrderMain(dto);//insertOrderMain을 호출하는 시점에서는 dto에 order_id 값이 없다.
		//insertOrderMain을 수행하면서 selectKey가 dto에 order_id 값을 입력한다.
		if(successCount < 1) return successCount;

		if(buyNowYN == 0) {
			successCount = dao.insertOrderDetail(dto);//insertOrderDetail을 호출하는 시점에서는 dto에 order_id 값이 있다.
			if(successCount < 1) return successCount;
